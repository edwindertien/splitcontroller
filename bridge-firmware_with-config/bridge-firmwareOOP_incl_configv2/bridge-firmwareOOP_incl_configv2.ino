...
vector<PinAllocation> pinalloc =
{
  {"joyRXax2", 2, DIGITAL, "foo"},
  {"joyRYax1", 3, DIGITAL, "foo"},
  {"joyRZb11", 4, DIGITAL,  "foo"},
  {"joyLZb12", 7, DIGITAL,  "foo"},
  {"joyLYax4", 6, DIGITAL,  "foo"},
  {"joyLXax3", 5, DIGITAL,  "foo"},
  {"sqbutton", 26, DIGITAL,  "foo"},
  {"circbutton", 28, DIGITAL,  "foo"},
  {"psbutton", 30, DIGITAL,  "foo"},
  {"l2button", 32, DIGITAL,  "foo"},
  {"r2button", 34, DIGITAL,  "foo"},
  {"tributton", 36, DIGITAL,  "foo"},
  {"xbutton", 38, DIGITAL,  "foo"},
  {"lhat", 40, DIGITAL,  "foo"},
  {"rhat", 42, DIGITAL, "foo"},
  {"uhat", 44, DIGITAL, "foo"},
  {"dhat", 46, DIGITAL, "foo"},
  {"r1button6", 48, DIGITAL,  "foo"},
  {"l1button5", 50, DIGITAL, "foo"},
  {"optbutton",  52, DIGITAL, "foo"},
  {"shrebutton", A8, DIGITAL, "foo"},
  {"accx1", A5, ANALOG,  "foo"},
  {"accy1", A4, ANALOG,  "foo"},
  {"accx2", A1, ANALOG,  "foo"},
  {"accy2", A0, ANALOG,  "foo"},
  {"c_button1", A6, ANALOG,  "foo"},
  {"z_button1", A7, ANALOG,  "foo"},
  {"c_button2", A2, ANALOG,  "foo"},
  {"z_button2", A3, ANALOG,  "foo"},
  {"PWM1", 2, PWM, "foo"},
  {"PWM2", 3, PWM, "foo"},
  {"PWM3", 5, PWM, "foo"},
  {"PWM4", 6, PWM, "foo"},
  ...
  class Contr {
    protected:
      ...
      void init() {
        ...
        // TODO: check if this is right for all items in list
        for (it = m_ps4digipins.begin(); it != m_ps4digipins.end(); it++)
        {
        ...
        digitalWrite(m_ps4digipins[0]["joyRZb11"], LOW); // hoog actief   4
        digitalWrite(m_ps4digipins[0]["joyLZb12"], LOW); // hoog actief 7
        digitalWrite(m_ps4digipins[0]["r1button6"], LOW);// hoog actief  48
        digitalWrite(m_ps4digipins[0]["l1button5"], LOW);// hoog actief  50

        analogWrite(m_ps4pwmpins[0]["PWM1"], 127);  // start with centervoltage
        analogWrite(m_ps4pwmpins[0]["PWM2"], 127);
        analogWrite(m_ps4pwmpins[0]["PWM3"], 127);
        analogWrite(m_ps4pwmpins[0]["PWM4"], 127);

        pinMode(m_ps4analogpins[0]["c_button2"], INPUT_PULLUP);  // for the switches.
        pinMode(m_ps4analogpins[0]["z_button2"], INPUT_PULLUP);
        pinMode(m_ps4analogpins[0]["c_button1"], INPUT_PULLUP);
        pinMode(m_ps4analogpins[0]["z_button1"], INPUT_PULLUP);
